...
router.post('/getEmploymentInfo', function(req, res) {
    mysql_pool.getConnection(function(err, connection) {
        ...
        connection.query('SELECT TimeCommitment, isInternationalStudent, SpeakTest, isTA, isGrader, CurrentEmployer, WorkHours FROM Application WHERE ASURITE_ID = ?', [req.body.user], function(err2, rows) {
            if(err2) {
                console.log('Error performing query: ' + err2);
                throw err2;
            } else if (!rows.length) {
                ...
                res.send({'TimeCommitment' : rows[0].TimeCommitment,'isInternationalStudent' : rows[0].isInternationalStudent, 'SpeakTest' : rows[0].SpeakTest,  'isTA' : rows[0].isTA, 'isGrader' : rows[0].isGrader, 'CurrentEmployer' : rows[0].CurrentEmployer, 'WorkHours' : rows[0].WorkHours});
            } 
            connection.release();
        });
    });
