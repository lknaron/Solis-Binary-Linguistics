...
router.post('/', function(req, res) {
    mysql_pool.getConnection(function(err, connection) {
        ...
        connection.query('SELECT * FROM Course_Competencies WHERE ASURITE_ID = ?', [req.body.ASURITE_ID], function(err2, rows) { 
            if(err2) {
                console.log('Error performing query: ' + err2);
                throw err2;
            } else if (!rows.length) {
                connection.query('INSERT INTO Course_Competencies SET ?', [req.body], function(err3) {
                    if(err3) {
                ...
                connection.query('UPDATE Course_Competencies SET ? WHERE ASURITE_ID = ?', [req.body, req.body.ASURITE_ID], function(err4) {
                    if(err4) {
...
// Checks if user already has this information saved then Saves/Updates user entered information into application
router.post('/coursesTaught', function(req, res) {
    mysql_pool.getConnection(function(err, connection) {
        if (err) {
            connection.release();
            console.log('Error getting mysql_pool connection: ' + err);
...
router.post('/getCoursesInfo', function(req, res) {
    mysql_pool.getConnection(function(err, connection) {
        ...
        connection.query('SELECT * FROM Course_Competencies WHERE ASURITE_ID = ?', [req.body.user], function(err2, rows) {
            if(err2) {
                console.log('Error performing query: ' + err2);
                throw err2;
            } else if (!rows.length) {
...
// Returns data to populate application page if user already saved courses taught information
router.post('/getCoursesTaughtInfo', function(req, res) {
    mysql_pool.getConnection(function(err, connection) {
        if (err) {
            connection.release();
            console.log('Error getting mysql_pool connection: ' + err);
