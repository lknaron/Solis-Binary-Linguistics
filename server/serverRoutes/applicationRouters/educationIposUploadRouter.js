            ...
            if (err) {
                ...
                connection.query('SELECT IposName FROM Attachment WHERE ASURITE_ID = ?', [req.user.username], function(err2) {
                    if(err2) {
                        ...
                        connection.query('INSERT INTO Attachment (IposName, UploadDate, ASURITE_ID) VALUES (?, ?, ?)', [req.files[0].originalname, uploadTime, req.user.username], function(err3) { 
                            if(err3) {
                        ...
                        connection.query('UPDATE Attachment SET IposName = ?, UploadDate = ? WHERE ASURITE_ID = ?', [req.files[0].originalname, uploadTime, req.user.username], function(err4) { 
                            if(err4) {
