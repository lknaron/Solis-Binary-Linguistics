    ...
    if (req.files) {
        mysql_pool.getConnection(function(err, connection) {
            if (err) {
                ...
                connection.query('SELECT TranscriptName FROM Attachment WHERE ASURITE_ID = ?', [req.user.username], function(err2) {
                    if(err2) {
                        console.log('Error performing query: ' + err2);
                        throw err2;
                    } else if (!rows.length) {
                        connection.query('INSERT INTO Attachment (TranscriptName, UploadDate, ASURITE_ID) VALUES (?, ?, ?)', [req.files[0].originalname, uploadTime, req.user.username], function(err3) { 
                            if(err3) {
                        ...
                        connection.query('UPDATE Attachment SET TranscriptName = ?, UploadDate = ? WHERE ASURITE_ID = ?', [req.files[0].originalname, uploadTime, req.user.username], function(err4) { 
                            if(err4) {
