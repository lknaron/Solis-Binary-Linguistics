...
router.post('/', function(req, res) {
    ...
    if (req.files) {
        mysql_pool.getConnection(function(err, connection) {
            ...
            connection.query('DELETE FROM Attachment WHERE ASURITE_ID = ? AND AttachmentType = "Resume"', [req.user.username], function(err3) {
                if(err3) {
                    console.log('Error performing query: ' + err3);
                    throw err3;
                } else {
                    connection.query('INSERT INTO Attachment (AttachmentName, AttachmentType, UploadDate, ASURITE_ID) VALUES (?, "Resume", ?, ?)', [req.files[0].originalname, uploadTime, req.user.username], function(err4) { 
                        if(err4) {
